...
Design notes, the reasoning behind the choices that are not obvious, and how
to build and test. For installing and using OpenFrp see the
[README](../README.md).
...
[the benchmark](benchmark.md), which also records the prediction this
project got wrong.
...
[`benchmark.md`](benchmark.md) publishes the numbers **including the
...
make bundle      # the router-side bundles the updater installs
```
...
scripts/                 install.sh and the release bundler
internal/
  tunnel/
  ...
  update/                release checks and self-update
...
| **P2** | OpenWrt package and LuCI app | ✅ done |
...
- **KCP, QUIC, WebSocket transports and Secret TCP have been removed.** The
  interface offered all four; none was ever implemented. Measured on the link
  this runs over — 0% loss, 50 ms, 1 ms jitter — KCP had nothing to recover
  and QUIC no head-of-line blocking to remove, so they were deleted rather
  than left as settings that do nothing.
...
[`test-environments.md`](test-environments.md) with the constraints
they impose — including that the router runs OpenWrt 25.12, so packages must be
`.apk` rather than `.ipk`.

## Rate limiting without losing splice

A limit is applied as a bounded `splice(2)` rather than a copy through
userspace: the kernel call takes a byte count, and Go passes one through when
the source is an `io.LimitedReader` wrapping a TCP connection. Limiting by
reading into a buffer would have cost the zero-copy path for every limited
tunnel, which is the one property the data plane is built around.
`TestLimitedRelayKeepsTheKernelFastPath` asserts a paced relay still reports
itself spliced.

A tunnel's limiter is built once and shared by every connection of that
tunnel. Attaching one per connection would quietly turn a per-tunnel rate into
a per-connection rate — ten visitors getting a megabyte each rather than a
megabyte between them.

## Counting traffic while it moves

Bytes are recorded as they move, not when a relay returns. A connection held
open by keep-alive read as zero for as long as it lived, which is most of the
time anyone is looking at the status page, and a quota went unspent while it
was being used.

The mechanism is a read deadline, not a smaller chunk. A bounded splice blocks
until the bound fills — 100 bytes against a 1 MiB bound simply waits — so
chunking cannot produce a live figure. Bounding the round by time instead
makes splice return what it moved, already delivered, and the next round
starts. Measured cost: no throughput difference and four allocations per
connection, because a busy transfer never reaches the deadline.

A yamux stream reports its own timeout type rather than the one a TCP deadline
produces, so the check is on `net.Error` — an overflow-carried connection must
not be torn down for a deadline we set on it ourselves.

## Releasing

Set the number in `internal/version/version.go`, commit, then tag:

```bash
git tag v0.4.0 && git push --tags
```

`.github/workflows/release.yml` cross-compiles, builds one bundle per
architecture with `scripts/bundle.sh`, publishes `checksums.txt` alongside
them, and refuses to release when the tag and the version the source declares
disagree — a binary reporting a number other than its tag would offer itself
as an upgrade forever.

The updater refuses a release with no `checksums.txt`, takes the expected hash
from the release rather than the download, unpacks only into an allowlisted
set of paths, runs the downloaded client once before replacing anything, and
restores every replaced file if the service does not come back.
`TestReleaseBundleIsInstallable` builds a real bundle and installs it, because
the bundler and the installer are one agreement about layout written twice.
